...
const REGISTRATION_REQUESTS_ENDPOINT = "/api/registration-requests";
...
async function submitRegistrationRequest(event) {
  ...
  registerFeedback.textContent = "正在提交申请…";
  try {
    const response = await fetch(REGISTRATION_REQUESTS_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, account, password, email, message })
    });
    const result = await response.json();
    if (!response.ok || result.code !== 20000) throw new Error(result.message || "提交失败");
    registerForm.reset();
    registerFeedback.textContent = "申请已提交，等待管理员审核。";
    setTimeout(closeRegisterModal, 900);
  } catch (error) {
    registerFeedback.textContent = error.message || "提交失败，请稍后重试。";
  }
}
