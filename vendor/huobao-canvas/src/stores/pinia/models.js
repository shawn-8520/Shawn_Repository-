...
const STORAGE_KEYS = {
  ...
  API_KEY_STORAGE_MODE: 'api-key-storage-mode',
  ...
  SESSION_API_KEYS_BY_PROVIDER: 'session-api-keys-by-provider',
...
const getSessionJson = (key, defaultValue = []) => {
  try {
...
const hasValues = (value) => value && Object.values(value).some(Boolean)
...
export const useModelStore = defineStore('model', () => {
  ...
  const apiKeyStorageMode = ref(getStored(STORAGE_KEYS.API_KEY_STORAGE_MODE, 'session'))
  ...
  const apiKeysByProvider = ref(
    getSessionJson(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER, null)
    || getStoredJson(STORAGE_KEYS.API_KEYS_BY_PROVIDER, {})
  )
  ...
  const setApiKeyStorageMode = (mode) => {
    const nextMode = mode === 'persistent' ? 'persistent' : 'session'
    apiKeyStorageMode.value = nextMode
    setStored(STORAGE_KEYS.API_KEY_STORAGE_MODE, nextMode)
  }
  ...
  const clearLegacyApiSecrets = () => {
    removeStored('apiKey')
    removeStored(STORAGE_KEYS.API_KEYS_BY_PROVIDER)
    removeSession(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER)
  ...
  const getImageEndpoint = (modelKey = '') => {
    const endpointConfig = providerConfig.value.endpoints?.image || '/images/generations'
    const endpoint = typeof endpointConfig === 'function' ? endpointConfig(modelKey) : endpointConfig
  ...
  // 监听并持久化选中的模型
  watch(apiKeyStorageMode, (val) => {
    setStored(STORAGE_KEYS.API_KEY_STORAGE_MODE, val)
    if (val === 'session') {
      if (hasValues(apiKeysByProvider.value)) {
        setSessionJson(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER, apiKeysByProvider.value)
      } else {
        removeSession(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER)
      }
      removeStored(STORAGE_KEYS.API_KEYS_BY_PROVIDER)
      ...
      if (hasValues(apiKeysByProvider.value)) {
        setStoredJson(STORAGE_KEYS.API_KEYS_BY_PROVIDER, apiKeysByProvider.value)
      } else {
        removeStored(STORAGE_KEYS.API_KEYS_BY_PROVIDER)
      }
      removeSession(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER)
  ...
  // 监听并持久化 API 配置
  watch(apiKeysByProvider, (val) => {
    if (apiKeyStorageMode.value === 'persistent') {
      if (hasValues(val)) {
        setStoredJson(STORAGE_KEYS.API_KEYS_BY_PROVIDER, val)
      } else {
        removeStored(STORAGE_KEYS.API_KEYS_BY_PROVIDER)
      }
      removeSession(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER)
      ...
      if (hasValues(val)) {
        setSessionJson(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER, val)
      } else {
        removeSession(STORAGE_KEYS.SESSION_API_KEYS_BY_PROVIDER)
      }
      removeStored(STORAGE_KEYS.API_KEYS_BY_PROVIDER)
