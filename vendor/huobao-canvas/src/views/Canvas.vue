        ...
        <div class="bg-[var(--bg-primary)] rounded-xl border border-[var(--border-color)] p-3">
          ...
          <div class="flex items-center justify-between mt-2">
            <div class="flex items-center gap-2">
              <n-select
                v-model:value="inputChatModel"
                :options="inputChatModelOptions"
                size="small"
                filterable
...
import { NIcon, NSwitch, NDropdown, NMessageProvider, NSpin, NModal, NInput, NButton, NSelect } from 'naive-ui'
import { 
  ChevronBackOutline,
  ChevronDownOutline,
  SettingsOutline,
...
const inputChatModel = ref(modelStore.selectedChatModel || modelStore.availableChatModels[0]?.key || 'gpt-4o-mini')
const inputChatModelOptions = computed(() => modelStore.chatModelOptions.map(model => ({
  label: model.label,
  value: model.key
})))
const inputImageModel = computed(() => {
  const selected = modelStore.selectedImageModel
  const availableModels = modelStore.availableImageModels
  return availableModels.some(model => model.key === selected)
    ? selected
    : availableModels[0]?.key
})

watch(() => modelStore.selectedChatModel, (newModel) => {
  if (newModel) inputChatModel.value = newModel
})

watch(() => modelStore.currentProvider, () => {
  const availableModels = modelStore.availableChatModels
  if (!availableModels.some(model => model.key === inputChatModel.value)) {
    inputChatModel.value = modelStore.selectedChatModel || availableModels[0]?.key || 'gpt-4o-mini'
  }
})
...
const nodeTypeOptions = [
  ...
  { type: 'imageConfig', name: '文生图配置', icon: ColorPaletteOutline, color: '#8ea3b8' },
...
const handlePolish = async () => {
  ...
  try {
    ...
    const result = await sendChat(input, true, { model: inputChatModel.value })
  ...
  try {
    ...
    if (autoExecute.value) {
      ...
      try {
        ...
        const result = await analyzeIntent(content, inputChatModel.value)
