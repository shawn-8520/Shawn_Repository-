    ...
    <main class="max-w-5xl mx-auto px-4 py-8 md:py-16">
      ...
      <section class="text-center mb-12">
        <div class="flex items-center justify-center gap-4 mb-8">
          <h1 class="text-2xl md:text-4xl font-bold text-[var(--text-primary)]">欢迎来到 AI 无限画布</h1>
        </div>
          ...
          <div class="bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)] p-4 shadow-sm">
            ...
            <div class="flex items-center justify-between mt-2">
              <div class="flex items-center gap-2">
                <n-select
                  v-model:value="homeChatModel"
                  :options="homeChatModelOptions"
                  size="small"
                  filterable
...
const homeChatModel = ref(modelStore.selectedChatModel || modelStore.availableChatModels[0]?.key || 'gpt-4o-mini')
const homeChatModelOptions = computed(() => modelStore.chatModelOptions.map(model => ({
  label: model.label,
  value: model.key
})))

watch(homeChatModel, (newModel) => {
  if (newModel) modelStore.selectedChatModel = newModel
})

watch(() => modelStore.selectedChatModel, (newModel) => {
  if (newModel && homeChatModel.value !== newModel) {
    homeChatModel.value = newModel
  }
})

watch(() => modelStore.currentProvider, () => {
  const availableModels = modelStore.availableChatModels
  if (!availableModels.some(model => model.key === homeChatModel.value)) {
    homeChatModel.value = modelStore.selectedChatModel || availableModels[0]?.key || 'gpt-4o-mini'
  }
})
