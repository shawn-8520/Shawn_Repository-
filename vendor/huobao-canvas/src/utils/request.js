...
instance.interceptors.request.use(
  (config) => {
    ...
    // Get API keys from secure session storage first | 优先从会话存储读取 API Keys
    let apiKey = ''
    try {
      const apiKeysJson = sessionStorage.getItem('session-api-keys-by-provider')
        || localStorage.getItem('api-keys-by-provider')
