...
export const PROVIDERS = {
  ...
  openrouter: {
    ...
    responseAdapter: {
      ...
      image: (response) => {
        const data = response.data || response.images || response
        return (Array.isArray(data) ? data : [data]).map(item => ({
          url: item.url || item.b64_json || item.image_url || '',
          revisedPrompt: item.revised_prompt || ''
        }))
      }
    }
  ...
  huggingface: {
    label: 'Hugging Face 免费额度',
    defaultBaseUrl: 'https://router.huggingface.co',
    endpoints: {
      chat: '/v1/chat/completions',
      image: (model) => `/hf-inference/models/${String(model || '').split('/').map(encodeURIComponent).join('/')}`,
      video: '/v1/videos',
      videoQuery: '/v1/videos/{taskId}'
    },
    requestAdapter: {
    ...
    responseAdapter: {
      ...
      image: (response) => {
        if (response instanceof Blob) {
          return [{ url: URL.createObjectURL(response), revisedPrompt: '' }]
        }
